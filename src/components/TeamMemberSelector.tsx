...
export function TeamMemberSelector({
  ...
  const getTeamMembers = () => {
    ...
    const noneOption = { id: "none", username: "No User Head" } as User;
    
    if (showNoneOption) {
      teamMembers = [noneOption, ...teamMembers];
    }
    
    return teamMembers;
  };

  // Get and ensure teamMembers is always an array
  const teamMembers = getTeamMembers() || [];
  
  // Filter the team members based on search value, ensuring result is always an array
  const filteredTeamMembers = searchValue && teamMembers.length > 0
  ...
  // Ensure we have a valid selectedUser object
  const safeSelectedUser = selectedUser || { id: "none", username: "No User Head" };
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={isDisabled}
        >
          {safeSelectedUser?.username || safeSelectedUser?.login || placeholder}
          ...
          {filteredTeamMembers && filteredTeamMembers.length > 0 ? (
            <CommandGroup>
              {filteredTeamMembers.map((user) => {
                // Skip rendering if user is null or undefined
                if (!user) return null;
